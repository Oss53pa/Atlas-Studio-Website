...
import { Proph3tError } from "./types.js";
import type { Proph3tClientOptions } from "./types.js";
...
export async function postEdge<T>(
  ctx: HttpContext,
  functionName: string,
  body: unknown,
): Promise<T> {
  ...
  try {
    ...
    if ((err as Error).name === "AbortError") {
      throw new Proph3tError(
        `Timeout after ${ctx.timeoutMs}ms`,
        408,
        functionName,
      );
    }
...
export async function getRest<T>(
  ctx: HttpContext,
  path: string,
): Promise<T> {
