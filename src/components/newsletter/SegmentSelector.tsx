    ...
    <div className="space-y-2">
      ...
      {segments.map(seg => (
        <button key={seg.id} onClick={() => toggle(seg.id)}
          className={`w-full text-left p-3 rounded-lg border transition-colors ${selectedIds.includes(seg.id) ? 'border-p-accent bg-p-accent/10' : 'border-p-border bg-p-surface-alt/30 hover:bg-p-surface-alt'}`}
        >
