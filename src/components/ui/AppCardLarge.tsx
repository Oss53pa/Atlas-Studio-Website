    ...
    >
      <div className={`flex flex-col lg:flex-row ${reverse ? "lg:flex-row-reverse" : ""}`}>
        ...
        <div className="flex-1 p-8 md:p-10">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex items-center gap-1.5">
              {isComingSoon && (
                <span className="flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] font-semibold border bg-amber-500/10 text-amber-400 border-amber-500/20">
          ...
          <p className="text-neutral-placeholder text-[15px] leading-relaxed mb-6 font-light">{app.desc}</p>

          {highlights.length > 0 && (
            <div className="flex gap-2 flex-wrap mb-6">
              {highlights.map((h, i) => (
                <span key={i} className="px-3 py-1.5 rounded-lg text-[12px] font-semibold border" style={{ color: appColor, backgroundColor: `${appColor}15`, borderColor: `${appColor}30` }}>
                  {h}
                </span>
              ))}
            </div>
          ...
          <div className="flex items-end gap-6 flex-wrap mb-6">
            {prices.map(([planName, price]) => (
              <div key={planName}>
                <div className="text-neutral-muted text-[11px] font-semibold uppercase tracking-wider mb-0.5">{planName}</div>
                <div className="flex items-baseline gap-1">
                  {price === 0 ? (
                    <span className="text-gold font-mono text-2xl font-semibold">Gratuit</span>
                  ) : (
                    <>
                      <span className="text-gold font-mono text-2xl font-semibold">{formatPrice(price)}</span>
          ...
          <Link to={`/applications/${app.id}`} className="inline-flex items-center gap-2 bg-gold text-onyx px-6 py-3 rounded-lg font-semibold text-sm hover:bg-gold-dark transition-colors">
        ...
        <div className="lg:w-[380px] xl:w-[420px] bg-dark-bg3 p-8 md:p-10 border-t lg:border-t-0 lg:border-l border-dark-border flex flex-col justify-center">
          <div className="text-neutral-muted text-[11px] font-bold uppercase tracking-wider mb-4">Fonctionnalités clés</div>
            ...
            {previewFeatures.map((f, i) => {
              ...
              return (
                <div key={i} className="flex items-start gap-2.5">
                  ...
                  <span className="text-neutral-light text-[13px] font-light leading-snug">{clean}</span>
                </div>
              );
            })}
          </div>
          {moreCount > 0 && (
            <Link to={`/applications/${app.id}`} className="mt-4 text-[13px] font-semibold hover:gap-2 transition-all duration-200 inline-flex items-center gap-1" style={{ color: appColor }}>
