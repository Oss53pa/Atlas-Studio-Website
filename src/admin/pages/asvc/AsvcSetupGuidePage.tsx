...
export default function AsvcSetupGuidePage() {
  ...
  // Proxies: si on a un audit_log avec des entries, c'est que les migrations
  // sont au moins partiellement passées (les triggers d'audit existent).
  const migrationsLikelyDone = (health?.audit_log?.total_entries ?? 0) > 0;
  // Proxy edge functions : si on a réussi à fetch health, c'est que les
  // RPC SQL répondent — on ne peut pas tester les edge functions sans appel.
  const edgeFunctionsLikelyDeployed = !!health;

  const hasToken = (provider: string) =>
    tokens.some((t) => t.provider === provider && t.status === 'active');

  // Statuts dynamiques connecteurs
    ...
    <div className="max-w-6xl">
      ...
      <div className="mb-6 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4 text-[12.5px] text-amber-200">
        ...
        <p className="mt-1 text-amber-100/80 leading-relaxed">
          ...
          Ce guide te dit exactement quoi faire dans quel ordre.
        </p>
      </div>

      {/* Étape 1 — Infra critique */}
      <SetupSection
        n={1}
        title="Infrastructure critique"
        subtitle="Sans ça, les agents ne tournent pas du tout"
      >
        <Step
          icon={KeyRound}
          title="Anthropic API key"
          status="manual"
          desc="Crée une clé sur console.anthropic.com avec carte bancaire active. Les 19 agents font des appels Claude payants (~0.01-0.05$ par draft). Configure ANTHROPIC_API_KEY côté Supabase Edge Functions → Secrets."
        ...
        <Step
          ...
          status="manual"
          desc="Génère une chaîne aléatoire 32+ caractères. Configure APP_ENCRYPTION_KEY côté Supabase Edge Functions → Secrets. Sert à chiffrer tous les tokens OAuth/PAT en DB."
        />
        <Step
          icon={Activity}
          title="Cron shared secret"
          status="manual"
          desc="Génère une autre chaîne aléatoire 32+ caractères. Configure CRON_SHARED_SECRET côté Supabase ET côté GitHub Actions secrets. Permet aux crons d'appeler les edge functions sans JWT."
        />
        <Step
          icon={Database}
      ...
      <SetupSection
        n={2}
        title="Connecter les services externes"
        subtitle="Chaque connecteur débloque des capacités d'exécution. Sans connecteur, les agents restent en mode 'draft'."
      >
      ...
      <SetupSection
        n={3}
        title="Alimenter en données réelles"
        subtitle="Les agents ne créent pas de leads — ils les traitent. Tu dois leur donner du grain à moudre."
      >
        <Step
          icon={UserSearch}
      ...
      <SetupSection
        n={4}
        title="Itérer sur la qualité des agents"
        subtitle="Les prompts par défaut sont fonctionnels mais génériques. La vraie valeur vient du tuning sur tes vrais cas."
      >
        <Step
          icon={Settings}
      ...
      {/* Capacités actuelles des agents */}
      <h2 className="text-neutral-light text-[16px] font-semibold mt-10 mb-3">
      ...
      <p className="text-neutral-400 text-[12.5px] mb-4 leading-relaxed">
        ...
        connecteurs configurés ci-dessus.
      </p>
      ...
      <div className="space-y-5">
        <AgentDept title="🏛️ Direction" agents={[
          {
            code: 'coo',
            name: 'COO Agent',
            does: 'Synthétise briefs matin (07h) / soir (18h) / hebdo (lundi 06h30). Pas de LLM call direct — templates avec données live.',
            needs: 'Données dans la DB (actions, tickets, leads, etc.)',
        ...
        <AgentDept title="🛒 Ventes" agents={[
          {
            ...
            does: 'Qualifie BANT (Budget/Authority/Need/Timeline) sur un lead, identifie fits produit, suggère next stage (MQL/SQL).',
            needs: 'Leads dans asvc_leads. Apollo (optionnel — enrichit avec titre/société/effectif).',
            status: 'ready_internal',
          },
          {
            code: 'sdr',
            name: 'SDR Agent',
            does: 'Drafts emails/DMs LinkedIn/WhatsApp d\'outreach (first touch, follow-ups, breakup).',
            needs: 'Pour ship réel : Gmail / LinkedIn / WhatsApp configurés.',
            status: 'ship_required',
          },
          {
            code: 'closer',
            name: 'Closer Agent',
            does: 'Drafts propositions commerciales structurées (problème, solution, prix, échéancier).',
            needs: 'Pour ship réel : Gmail configuré.',
            status: 'ship_required',
          },
        ]} />
        ...
        <AgentDept title="💼 SAV" agents={[
          {
            code: 'support_n1',
            name: 'Support Agent N1',
            does: 'Drafts réponses tickets (FAQ, troubleshooting, how-to). Détecte triggers d\'escalade.',
            needs: 'Tickets dans asvc_tickets. Pour ship : Gmail (si source=email) ou WhatsApp (si source=whatsapp).',
            status: 'ship_required',
          },
          {
            code: 'customer_success',
            name: 'Customer Success Agent',
            does: 'Drafts emails lifecycle : onboarding J+1/J+7/J+30, churn check, upsell.',
            needs: 'Clients dans asvc_clients avec stage (d1/d7/d30/churn_risk/upsell). Pour ship : Gmail.',
            status: 'ship_required',
          },
          {
            code: 'bug_triage',
            name: 'Bug Triage Agent',
            does: 'Qualifie bug (P0/P1/P2/P3), drafts issue GitHub formatée avec repro/logs.',
            needs: 'Tickets type bug + error_logs. Pour ship : GitHub PAT.',
            status: 'ship_required',
          },
        ]} />
        ...
        <AgentDept title="📢 Marketing" agents={[
          {
            code: 'content',
            name: 'Content Agent',
            does: 'Drafts posts LinkedIn / X / Instagram / Facebook / newsletter / article blog selon brand voice.',
            needs: 'Pour ship réel : LinkedIn (UGC) ET/OU Meta (FB/IG). X = payant non câblé.',
            status: 'ship_required',
          },
          {
            code: 'community',
            name: 'Community Agent',
            does: 'Modère commentaires/DMs sociaux, escalade troll/sensible, drafts réponses.',
            needs: 'Pour ship : Meta (FB/IG comments). Aujourd\'hui mode draft uniquement.',
            status: 'ship_required',
          },
        ]} />
        ...
        <AgentDept title="💰 Finance" agents={[
          {
            code: 'facturation',
            name: 'Facturation Agent',
            does: 'Drafts relances graduées (level 1 friendly → level 5 mise en demeure). Génère lien paiement.',
            needs: 'Factures dans asvc_invoices. Pour ship : Gmail + CinetPay (UEMOA/CEMAC) ou Stripe (international).',
            status: 'ship_required',
          },
          {
            code: 'compta',
            name: 'Compta Agent',
            does: 'Suggère écritures SYSCOHADA pour chaque facture (compte/libellé/débit/crédit). Validation CEO avant import Atlas Finance.',
            needs: 'Factures dans asvc_invoices. Pas de connecteur — sortie = JSON pour import manuel.',
            status: 'ready_internal',
          },
          {
            code: 'tresorerie',
            name: 'Tresorerie Agent',
            does: 'Brief trésorerie quotidien (cash position, runway, alertes seuils, recommandations).',
            needs: 'Snapshot finance dans la DB. Tourne en cron daily 06h UTC.',
            status: 'ready_internal',
          },
        ]} />
        ...
        <AgentDept title="🔬 R&D / Production" agents={[
          {
            code: 'veille',
            name: 'Veille Agent',
            does: 'Qualifie une opportunité (new_app / new_feature / pivot / integration) avec scoring RICE.',
            needs: 'Signal en input (texte libre). 100% LLM — pas de connecteur.',
            status: 'ready_internal',
          },
          {
            code: 'user_research',
            name: 'User Research Agent',
            does: 'Approfondit une opportunité validée : research brief, feedbacks SAV, template interview.',
            needs: 'Opportunité dans asvc_opportunities + tickets associés.',
            status: 'ready_internal',
          },
          {
            code: 'product_designer',
            name: 'Product Designer Agent',
            does: 'Produit spec complète (vision, user stories, archi, wireframes Mermaid).',
            needs: 'Opportunité + research brief. Sortie = asvc_product_specs.',
            status: 'ready_internal',
          },
          {
            code: 'dev',
            name: 'Dev Agent',
            does: 'Drafts plan d\'implémentation par fichiers, structure de PR, checklist tests QA.',
            needs: 'Spec validée. Pour ship réel : GitHub PAT (pour pousser le plan + ouvrir PR draft).',
            status: 'ship_required',
          },
          {
            code: 'qa',
            name: 'QA Agent',
            does: 'Génère plan de tests détaillés (unit / integration / e2e / sécurité / SYSCOHADA si finance).',
            needs: 'PR ou spec. Pas de connecteur — sortie = asvc_test_runs pour CI.',
            status: 'ready_internal',
          },
          {
            code: 'devops_release',
            name: 'DevOps/Release Agent',
            does: 'Pipeline deploy avec triple gate (QA passed + Preview CEO approved + 0 incident P0/P1). Confirmation typée obligatoire pour prod.',
            needs: 'Pour ship : Vercel PAT (deploy) + GitHub PAT (tags/rollback) + Sentry PAT (post-deploy monitor).',
            status: 'ship_required',
          },
          {
            code: 'documentation',
            name: 'Documentation Agent',
            does: 'Drafts MDX : user guide / API ref / changelog / release notes / admin guide / troubleshooting.',
            needs: 'App + version. Pour ship : GitHub PAT + ASVC_MINTLIFY_DOCS_REPO.',
            status: 'ship_required',
          },
        ]} />
      </div>
      ...
      <div className="mt-10 rounded-xl border border-admin-accent/30 bg-admin-accent/5 p-5">
        <h3 className="text-admin-accent text-[14px] font-semibold mb-2">Prochaines actions concrètes</h3>
        <ol className="text-neutral-300 text-[12.5px] space-y-1.5 list-decimal list-inside leading-relaxed">
          <li>Configure les <strong>3 secrets Supabase critiques</strong> (Anthropic, Encryption, Cron)</li>
          <li>Pousse les <strong>migrations</strong> et <strong>déploie les edge functions</strong></li>
          <li>Connecte <strong>1 connecteur</strong> pour commencer (recommandé : Gmail pour activer SAV + SDR + Facturation)</li>
          <li>Importe ou crée <strong>10 leads</strong> dans <code>asvc_leads</code> → lance Prospection sur le premier</li>
          <li>Regarde le draft → ajuste le prompt si besoin → valide → laisse SDR enchaîner</li>
  ...
  return (
    <section className="mb-8">
      <div className="flex items-baseline gap-3 mb-3">
        <span className="text-admin-accent text-[14px] font-bold font-mono">{n}.</span>
        <div>
          <h2 className="text-neutral-light text-[14.5px] font-semibold">{title}</h2>
          <p className="text-neutral-500 text-[11.5px]">{subtitle}</p>
        </div>
      </div>
      <div className="space-y-2.5 ml-7">{children}</div>
    </section>
  );
}
