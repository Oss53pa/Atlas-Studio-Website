    ...
    <div>
      <div className="flex items-center justify-between mb-7 flex-wrap gap-4">
        <div>
          ...
          <p className="text-neutral-muted text-sm">{tickets.length} tickets — {counts.open + counts.in_progress} en attente</p>
        </div>
      ...
      <AdminTable
        ...
        columns={[
          ...
          { key: "user", label: "Client", render: (r: TicketWithProfile) => (
            <div>
              <div className="text-neutral-text text-[13px]">{r.profiles?.full_name || "—"}</div>
          ...
          { key: "created_at", label: "Date", sortable: true, render: (r: TicketWithProfile) =>
            <span className="text-[12px] text-neutral-muted">{new Date(r.created_at).toLocaleDateString("fr-FR", { day: "2-digit", month: "short" })}</span>
          },
        ]}
        data={filtered}
      />
          ...
          activeTicket && activeTicket.status !== "closed" ? (
            <>
              <button onClick={() => handleReply(false)} disabled={sending || !reply.trim()}
                className={`btn-gold !py-2.5 !px-5 !text-[13px] flex items-center gap-2 ${sending || !reply.trim() ? "opacity-50" : ""}`}>
                <Send size={14} /> Répondre
              </button>
              <button onClick={() => handleReply(true)} disabled={sending || !reply.trim()}
                className={`px-5 py-2.5 border border-warm-border rounded-lg text-[13px] font-medium text-neutral-body hover:border-gold/40 transition-colors ${sending || !reply.trim() ? "opacity-50" : ""}`}>
                Répondre et fermer
              </button>
            </>
          ) : undefined
            ...
            <div className="flex-1 space-y-4 overflow-y-auto mb-4 min-h-[300px] max-h-[calc(100vh-380px)]">
              {messages.map(m => (
                <div key={m.id} className={`flex ${m.is_admin ? "justify-end" : "justify-start"}`}>
                  <div className={`max-w-[80%] p-4 rounded-2xl ${m.is_admin ? "bg-gold/5 border border-gold/10 rounded-br-md" : "bg-warm-bg border border-warm-border rounded-bl-md"}`}>
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className="text-[11px] font-semibold text-neutral-muted">
                        {m.is_admin ? "Admin" : activeTicket.profiles?.full_name || "Client"}
                      </span>
                      <span className="text-[10px] text-neutral-placeholder">
                        {new Date(m.created_at).toLocaleString("fr-FR", { day: "2-digit", month: "short", hour: "2-digit", minute: "2-digit" })}
                      </span>
                    </div>
                    <div className="text-neutral-text text-[13px] whitespace-pre-wrap leading-relaxed">{m.message}</div>
                  </div>
                </div>
              ))}
              {messages.length === 0 && (
                <div className="text-center text-neutral-muted text-sm py-8">Aucun message</div>
              )}
            </div>
