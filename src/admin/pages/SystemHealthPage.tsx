  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4">
        <div>
          <h1 className="text-neutral-text text-2xl font-bold mb-1">Santé système</h1>
          <p className="text-neutral-muted text-sm">Monitoring et état des services</p>
        </div>
        <div className="flex gap-2">
          <button onClick={checkServices} className="flex items-center gap-2 px-4 py-2.5 border border-warm-border rounded-lg bg-white text-neutral-body text-[13px] font-medium hover:border-gold/40 transition-colors">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-4 mb-8">
        <div className="lg:col-span-1 bg-white border border-warm-border rounded-xl p-6 flex flex-col items-center justify-center">
          ...
          <div className="text-neutral-text text-sm font-semibold">Score global</div>
        ...
        {services.map(s => (
          <div key={s.name} className="bg-white border border-warm-border rounded-xl p-5">
            <div className="flex items-center gap-3 mb-3">
              ...
              <div>
                <div className="text-neutral-text text-sm font-medium">{s.name}</div>
                <div className="text-neutral-muted text-[11px]">{s.description}</div>
              </div>
            </div>
            <div className="flex items-center justify-between">
              ...
              {s.responseTime > 0 && (
                <span className="text-neutral-muted text-[11px] font-mono">{s.responseTime}ms</span>
              )}
            </div>
          </div>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white border border-warm-border rounded-xl p-6">
          <h2 className="text-neutral-text text-sm font-semibold mb-4">Uptime (7 derniers jours)</h2>
        ...
        <div className="bg-white border border-warm-border rounded-xl p-6">
          <h2 className="text-neutral-text text-sm font-semibold mb-4">Incidents récents</h2>
          {incidents.length === 0 ? (
            <div className="flex items-center gap-3 py-6 justify-center">
              <CheckCircle2 size={20} className="text-emerald-500" />
              <span className="text-neutral-muted text-sm">Aucun incident récent</span>
            </div>
          ) : (
