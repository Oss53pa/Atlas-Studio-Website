...
export default function ActivityLogPage() {
  ...
  const getUserDisplay = (userId: string | null) => {
    if (!userId) return <span className="text-neutral-muted text-[12px]">Système</span>;
    const user = userMap[userId];
    if (!user) return <span className="text-neutral-muted text-[11px] font-mono">{userId.slice(0, 8)}...</span>;
    return (
      <div>
        <div className="text-neutral-text text-[13px] font-medium">{user.name}</div>
        <div className="text-neutral-muted text-[11px]">{user.email}</div>
      </div>
    );
  };
    ...
    <div>
      <div className="flex items-center justify-between mb-7 flex-wrap gap-4">
        <div>
          ...
          <p className="text-neutral-muted text-sm">Historique des actions sur la plateforme — {totalCount} événements</p>
        </div>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        ...
        <div className="relative">
          <Calendar size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-neutral-muted pointer-events-none" />
          <select value={dateFilter} onChange={e => { setDateFilter(e.target.value); setPage(0); }}
            className="pl-9 pr-4 py-2.5 bg-white border border-warm-border rounded-lg text-sm text-neutral-text outline-none focus:border-gold transition-colors appearance-none cursor-pointer">
            {DATE_FILTERS.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
          </select>
        </div>
        <select value={actionFilter} onChange={e => { setActionFilter(e.target.value); setPage(0); }}
          className="px-4 py-2.5 bg-white border border-warm-border rounded-lg text-sm text-neutral-text outline-none focus:border-gold transition-colors appearance-none cursor-pointer">
      ...
      <div className="bg-white border border-warm-border rounded-2xl overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full border-collapse">
            <thead>
              <tr className="border-b border-warm-border bg-warm-bg/30">
                <th className="text-neutral-muted text-[11px] font-bold uppercase tracking-wider p-4 text-left">Date</th>
                <th className="text-neutral-muted text-[11px] font-bold uppercase tracking-wider p-4 text-left">Action</th>
                <th className="text-neutral-muted text-[11px] font-bold uppercase tracking-wider p-4 text-left">Utilisateur</th>
                <th className="text-neutral-muted text-[11px] font-bold uppercase tracking-wider p-4 text-left">Détails</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={4} className="p-12 text-center text-neutral-muted text-sm">Aucun événement</td></tr>
              ) : filtered.map(log => (
                <tr key={log.id} onClick={() => setDetailLog(log)}
                  className="border-b border-warm-bg last:border-b-0 hover:bg-warm-bg/50 transition-colors cursor-pointer">
                  <td className="p-4 text-neutral-body text-[13px] whitespace-nowrap">
                  ...
                  <td className="p-4 text-neutral-muted text-[11px] max-w-[300px] truncate">
        ...
        <div className="flex items-center justify-between px-4 py-3 border-t border-warm-border">
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-[12px] text-neutral-muted px-2">{page + 1} / {Math.max(totalPages, 1)}</span>
      ...
      <AdminModal open={!!detailLog} onClose={() => setDetailLog(null)} title="Détail de l'événement" size="md">
        {detailLog && (
          <div className="space-y-5">
            <div>
              <div className="text-neutral-muted text-[11px] font-semibold uppercase tracking-wider mb-1">Date</div>
            ...
            <div>
              <div className="text-neutral-muted text-[11px] font-semibold uppercase tracking-wider mb-1">Action</div>
              {getActionBadge(detailLog.action)}
            </div>
            <div>
              <div className="text-neutral-muted text-[11px] font-semibold uppercase tracking-wider mb-1">Utilisateur</div>
              {getUserDisplay(detailLog.user_id)}
              {detailLog.user_id && (
                <div className="text-neutral-muted text-[10px] font-mono mt-1">{detailLog.user_id}</div>
              )}
            </div>
            <div>
              <div className="text-neutral-muted text-[11px] font-semibold uppercase tracking-wider mb-1">Metadata</div>
              <pre className="bg-warm-bg rounded-lg p-4 text-[12px] text-neutral-text font-mono overflow-auto max-h-[300px] whitespace-pre-wrap">
