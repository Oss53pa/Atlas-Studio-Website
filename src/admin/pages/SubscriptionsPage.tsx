  ...
  return (
    <div>
      ...
      <div className="flex gap-2 flex-wrap mb-6">
        <button onClick={() => setAppFilter("all")} className={`px-3 py-1.5 rounded text-[12px] font-medium transition-all ${appFilter === "all" ? "bg-neutral-200 text-neutral-700" : "bg-warm-bg text-neutral-muted hover:bg-neutral-100"}`}>
          Toutes les apps
        </button>
        {uniqueApps.map(appId => (
          <button key={appId} onClick={() => setAppFilter(appId)} className={`px-3 py-1.5 rounded text-[12px] font-medium transition-all ${appFilter === appId ? "bg-neutral-200 text-neutral-700" : "bg-warm-bg text-neutral-muted hover:bg-neutral-100"}`}>
      ...
      <AdminModal open={showForm} onClose={() => setShowForm(false)} title="Nouvel abonnement">
        <div className="space-y-3">
          ...
          <div>
            <label className="block text-neutral-body text-[13px] font-semibold mb-1.5">Statut initial</label>
      ...
      <AdminModal open={showEditForm} onClose={() => setShowEditForm(false)} title="Modifier l'abonnement">
        {editSub && (
          <div className="space-y-3">
            <div className="px-4 py-3 bg-warm-bg rounded-lg text-sm">
              <p className="font-semibold text-neutral-text">{editSub.profiles?.full_name}</p>
              <p className="text-neutral-muted text-[12px]">{editSub.profiles?.email} — {appMap[editSub.app_id]?.name || editSub.app_id}</p>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-neutral-body text-[13px] font-semibold mb-1.5">Plan</label>
                <input value={editData.plan} onChange={e => setEditData(p => ({ ...p, plan: e.target.value }))} className="w-full px-4 py-3 bg-warm-bg border border-warm-border rounded-lg text-neutral-text text-sm outline-none focus:border-gold transition-colors" />
              </div>
              <div>
                <label className="block text-neutral-body text-[13px] font-semibold mb-1.5">Prix (FCFA)</label>
                <input type="number" value={editData.price} onChange={e => setEditData(p => ({ ...p, price: Number(e.target.value) }))} className="w-full px-4 py-3 bg-warm-bg border border-warm-border rounded-lg text-neutral-text text-sm outline-none focus:border-gold transition-colors" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-neutral-body text-[13px] font-semibold mb-1.5">Fin essai</label>
                <input type="date" value={editData.trial_ends_at} onChange={e => setEditData(p => ({ ...p, trial_ends_at: e.target.value }))} className="w-full px-4 py-3 bg-warm-bg border border-warm-border rounded-lg text-neutral-text text-sm outline-none focus:border-gold transition-colors" />
              </div>
              <div>
                <label className="block text-neutral-body text-[13px] font-semibold mb-1.5">Fin periode</label>
                <input type="date" value={editData.current_period_end} onChange={e => setEditData(p => ({ ...p, current_period_end: e.target.value }))} className="w-full px-4 py-3 bg-warm-bg border border-warm-border rounded-lg text-neutral-text text-sm outline-none focus:border-gold transition-colors" />
              </div>
            </div>
            <button onClick={handleEditSub} disabled={saving} className={`btn-gold w-full mt-4 ${saving ? "opacity-50" : ""}`}>
