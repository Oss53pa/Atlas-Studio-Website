...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (paymentMethod === "cinetpay") {
      const transactionId = `reg_${crypto.randomUUID().replace(/-/g, "").slice(0, 20)}`;
      ...
      const result = await initPayment({
        ...
        currency: "XOF",
        transactionId,
        description: `Regularisation - ${sub.app_id} (${sub.plan})`,
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      metadata: { userId: user.id, appId: sub.app_id, plan: sub.plan, subscriptionId: sub.id, type: "regularization" },
